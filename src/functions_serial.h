...
*/
float F1(const float *x, const float *params)
{
...
/* Name = (round(exp(-x))-round(exp(y))*sin(z)
	...
	params = []  (no parameters)
*/
float F2(const float *x, const float *params)
{
...
*/
float F3(const float *x, const float *params)
{
...
*/
float F4(const float *x, const float *params)
{
...
/* Name = Powerful Pony
	...
	params= [] (no parameters)
*/
float F5(const float *x, const float *params)
{
...
/* Name = Sphere Shell
	...
	params= [radius,width]
*/
float F6(const float *x, const float *params)
{
...
float myfunc(const float*x, const float * params) {
	return x[0]+x[1] + x[2];
}
#endif
